 ...
 *   npx tsx src/cli.ts cards [dir]   # Export cards-export.md + cards-map.html to dir
 */
...
if (cmd === "reset" || cmd === "new") {
  ...
  const cardsDir = join(thisDir, "data/cards");
  ...
  type GEdge = { source: string; target: string; type: string; label?: string };
  ...
  for (const file of cardFiles) {
    ...
    for (const pm of posMatches) {
      if (pm.kind === "def") {
        ...
        // pm.id is the trigger card, currentCard is the consequence
    ...
    // Hidden state reads: state.hidden.xxx or state.hidden["xxx"]
...
${"</"}script>
  ...
  console.log(
    "Usage: npx tsx src/cli.ts [show|left|right|state|reset|auto <N>|cards [dir]]",
  );
  process.exit(1);
}
